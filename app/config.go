...
type config struct {
	...
	WeChat struct {
		AppId         string `mapstructure:"app_id"`
		Secret        string `mapstructure:"secret"`
		ApiRequestUrl string `mapstructure:"api_request_url"`
	} `mapstructure:"wechat"`
	Kuaidi100 struct {
	...
	RedisCountDown struct {
		AutoCancel int64 `mapstructure:"auto_cancel"`
		AutoFinish int64 `mapstructure:"auto_finish"`
	} `mapstructure:"redisCountDown"`
	OperatingActivity struct {
		KeyName     string `mapstructure:"key_name"`
		ActivityOn  string `mapstructure:"activity_on"`
		ActivityOff string `mapstructure:"ofactivity_of"`
	} `mapstructure:"operatingActivity"`
	RabbitMQ struct {
		Host     string `mapstructure:"host"`
		Port     string `mapstructure:"port"`
		User     string `mapstructure:"user"`
	...
	} `mapstructure:"inviteCode"`
	CacheProduct struct {
		CachePrefix        string `mapstructure:"cache_prefix"`
		CachePrefixForList string `mapstructure:"cache_prefix_for_list"`
	} `mapstructure:"cacheProduct"`
}
